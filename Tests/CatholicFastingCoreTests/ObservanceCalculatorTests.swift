...
final class ObservanceCalculatorTests: XCTestCase {
    ...
    func testAshWednesdayStaysMandatoryOnFreshInstallWithEligibilityTogglesOnly() {
        let freshInstall = RuleSettings(
            birthYear: 0,
            birthMonth: 0,
            birthDay: 0,
        ...
        let observances = ObservanceCalculator.makeCalendar(for: 2026, settings: freshInstall)
        let ashWednesday = observances.first(where: { $0.title == "Ash Wednesday" })
        let allSaints = observances.first(where: { $0.title == "All Saints" })

        XCTAssertEqual(ashWednesday?.obligation, .mandatory)
        XCTAssertEqual(ashWednesday?.kind, .fastAndAbstinence)
        XCTAssertEqual(allSaints?.obligation, .mandatory)
    }
    ...
    func testCanadaFridayPenanceUsesCCCBGuidanceAndCitations() {
        ...
        let observances = ObservanceCalculator.makeCalendar(for: 2026, settings: settings)
        let canadaFriday = observances.first {
            $0.title == "Friday Penance (Outside Lent)"
                && Calendar.gregorian.component(.month, from: $0.date) == 5
                && Calendar.gregorian.component(.day, from: $0.date) == 1
        }

        XCTAssertEqual(canadaFriday?.obligation, .mandatory)
        XCTAssertTrue((canadaFriday?.detail ?? "").localizedCaseInsensitiveContains("canada"))
        XCTAssertTrue((canadaFriday?.detail ?? "").localizedCaseInsensitiveContains("charity or piety"))
        XCTAssertTrue(canadaFriday?.citations.contains(where: { $0.authority == .cccb }) ?? false)
        XCTAssertTrue((canadaFriday?.rationale ?? "").localizedCaseInsensitiveContains("cccb"))
    }
    ...
    func testUnknownBirthYearUsesEligibilityTogglesForAgeBasedRules() {
        let settings = RuleSettings(
            birthYear: 0,
            isAge14OrOlderForAbstinence: false,
            isAge18OrOlderForFasting: false,
        ...
        XCTAssertEqual(ashWednesday?.obligation, .notApplicable)
