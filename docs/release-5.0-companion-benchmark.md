...
- Do not present App Store status, release version, ratings, or competitive scores without verifying current sources first. Cached search snippets are not enough; prefer App Store Connect or live Apple metadata, and cross-check the repo's `MARKETING_VERSION`.
