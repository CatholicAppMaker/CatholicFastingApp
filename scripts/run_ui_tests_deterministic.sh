...
IPHONE_TESTS=(
	...
	"CatholicFastingAppUITests/testFreshLaunchOnboardingQuoteReminderCanBeEnabledAndStillComplete"
...
extract_simulator_id() {
	local device_line="$1"
	printf '%s\n' "${device_line}" | sed -n 's/.*(\([A-F0-9-]\{36\}\)).*/\1/p'
}
...
resolve_destination() {
	...
	*)
	...
	if [[ -z "${SIMULATOR_ID}" ]]; then
		printf '%s\n' "Unable to resolve simulator for suite ${SUITE}." >&2
		return 1
	fi

	DESTINATION="platform=iOS Simulator,id=${SIMULATOR_ID}"
	printf '%s\n' "simulator,${SUITE},${SIMULATOR_ID}" >&2
}
...
run_one_test() {
	...
	for attempt in 1 2; do
		...
		while kill -0 "${pid}" >/dev/null 2>&1; do
			...
			if [[ "${elapsed}" -ge "${TIMEOUT_SECONDS}" ]]; then
				...
				if [[ "${attempt}" -eq 1 ]]; then
					printf '%s\n' "${test_name},RETRY,${elapsed}"
					printf '%s\n' "Timeout reached. Restarting simulator and retrying once."
					restart_simulator
					continue 2
				fi
				printf '%s\n' "${test_name},TIMEOUT,${elapsed}"
				printf '%s\n' "Last log lines:"
		...
		if grep -qiE "invalid device state|server died|coresimulator\\.simerror|mach error -308" "${log_path}" && [[ "${attempt}" -eq 1 ]]; then
			printf '%s\n' "${test_name},RETRY,${elapsed}"
			printf '%s\n' "Simulator transport issue detected. Restarting simulator and retrying once."
			restart_simulator
			continue
		fi
...
main() {
	...
	local -a tests
	case "${SUITE}" in
	iphone) tests=("${IPHONE_TESTS[@]}") ;;
	ipad) tests=("${IPAD_TESTS[@]}") ;;
	*)
		printf '%s\n' "Unknown SUITE=${SUITE}. Use iphone or ipad." >&2
		return 1
		;;
